...
export const generateFlowChartUsingMermaidCode = async (code, language) => {
    ...
    console.log("Mermaid Code: \n" + mermaidCodeForFlowChart);
    // generate flow chart and save to disk
    const flowChartData = await generateFlowChartAndWriteToDisk(mermaidCodeForFlowChart);

    if (!flowChartData.success) {
        throw new ApiError("Couldn't generate a flowchart.", 500);
    }
